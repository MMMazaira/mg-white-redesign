  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-white transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
      <div className="max-w-7xl mx-auto px-5 flex items-center justify-between h-20">
        ...
        {/* ── Desktop right: teléfono + lang + CTA ── */}
        <div className="hidden lg:flex items-center gap-3">

          {/* Switch de idioma — visible y claro */}
          <button
            onClick={toggle}
            className="text-sm font-bold border-2 border-navy/20 hover:border-gold hover:text-gold text-navy/70 rounded-lg px-3 py-2 transition-all duration-200"
            aria-label="Cambiar idioma"
          >
            {lang === 'es' ? 'EN' : 'ES'}
          </button>

          {/* Teléfono en desktop: solo texto copiable, NO abre app.
              En móvil (menú hamburguesa) sí usa tel: para llamar. */}
          <div
            className="flex items-center gap-2 bg-navy/5 border border-navy/15 text-navy text-sm font-semibold px-4 py-2.5 rounded-lg select-all cursor-text"
            title={lang === 'es' ? 'Copia este número' : 'Copy this number'}
          >
        ...
        {/* ── Mobile right: lang toggle + hamburguesa ── */}
        <div className="lg:hidden flex items-center gap-3">
          <button
            onClick={toggle}
            className="text-sm font-bold border-2 border-navy/20 hover:border-gold text-navy/70 rounded-lg px-3 py-1.5 transition-all"
            aria-label="Cambiar idioma"
          >
            {lang === 'es' ? 'EN' : 'ES'}
          </button>
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-300 bg-white border-t border-gray-100 ${menuOpen ? 'max-h-screen' : 'max-h-0'}`}>
        <nav className="px-5 pb-6 flex flex-col gap-1">
          ...
          <div className="pt-4 flex flex-col gap-3">
            {/* En móvil sí usamos tel: para que lance la llamada */}
